...
exports.init = function(grunt) {
  ...
  class SourceMapConcatHelper {
    constructor(options) {
      ...
      const file = path.relative(path.dirname(this.dest), this.files.dest).replace(/\\/g, '/');
      const generator = new SourceMapGenerator({
        file: file
      });
      this.file = file;
      this.generator = generator;
      this.addMapping = (genLine, genCol, orgLine, orgCol, source, name) => {
        if (!source) {
     ...
     */
    _forEachTokenPosition(src, filename, callback) {
      let genLine = this.line;
      let genCol = this.column;
      let orgLine = 1;
      let orgCol = 0;
    ...
    async addlines(src, filename) {
      const sourceMapRegEx = /\n\/[*/][@#]\s+sourceMappingURL=((?:(?!\s+\*\/).)*).*/;
      let relativeFilename = path.relative(path.dirname(this.dest), filename);
      ...
      if (sourceMapRegEx.test(src)) {
        ...
        const sourceMap = JSON.parse(sourceContent);
        const sourceMapConsumer = await new SourceMapConsumer(sourceMap);
        // Consider the relative path from source files to new sourcemap.
        const sourcePathToSourceMapPath = path.relative(path.dirname(this.dest), sourceMapDir);
        // Transfer the existing mappings into this mapping
        const initLine = this.line;
        const initCol = this.column;
        sourceMapConsumer.eachMapping(args => {
          let source;
          if (args.source) {
    ...
    url() {
      ...
      if (this.options.sourceMapStyle === 'inline') {
        const inlineMap = new Buffer(this._write()).toString('base64');
    ...
    _write() {
      // New sourcemap.
      const newSourceMap = this.generator.toJSON();
      // Return a string for inline use or write the map.
      if (this.options.sourceMapStyle === 'inline') {
        grunt.verbose.writeln('Source map for ' + chalk.cyan(this.files.dest) + ' inlined.');
        return JSON.stringify(newSourceMap, null, '');
      }
      grunt.file.write(this.dest, JSON.stringify(newSourceMap, null, ''));
  ...
  exports.helper = (files, options) => {
    ...
    if (options.sourceMapStyle === 'inline' && options.sourceMapName) {
      grunt.log.warn('Source map will be inlined, sourceMapName option ignored.');
    }
